...
if __name__ == '__main__':
    ...
    for emotion in EMOTION_LABELS_MAP.keys():
        emotion_tweets = [t[1] for t, s in zip(tweets.iteritems(),
                                               sentiment.iteritems())
                          if s[1] == emotion]
    ...
    smote_labels = numpy.array(smote_labels)
    x_train, x_test, y_train, y_test = train_test_split(trvs, smote_labels,
                                                        test_size=0.20,
                                                        random_state=SEED)
    ...
    report = classification_report(y_true=[y for y in y_test],
                                   y_pred=[pred for pred in predictions])
    print(conf_matrix)
    print(report)
